...
export class ItineraryPlannerService {
  ...
  private googlePhotoApiKey = process.env.GOOGLE_PHOTO_API_KEY;
  private googlePlacesApiBase = "https://maps.googleapis.com/maps/api/place";
  private googleMapsApiBase = "https://serpapi.com/search.json";
  private googleHotelApiBase = "https://serpapi.com/search.json";
  private googleTravelApiBase = "https://serpapi.com/search.json";
  ...
  async generateDetailedItinerary(
    ...
    try {
      ...
      const dailyItinerary = await this.generateSimpleDailyItinerary(
        destination, 
        duration, 
        preferences
      );
  ...
  private async getDestinationDetails(destination: string, preferences: UserPreferences): Promise<DestinationInfo> {
    ...
    try {
      ...
      const hotels = await Promise.all(
        (data.local_results || []).slice(0, 8).map(async (hotel: any) => {
          const photos = await this.getHotelPhotos(
            hotel.place_id || hotel.data_id, 
  ...
  private async getRestaurantsWithDetails(destination: string, preferences: UserPreferences): Promise<any[]> {
    try {
      ...
      return await Promise.all(
        (data.local_results || []).slice(0, 10).map(async (restaurant: any) => {
          const photos = await this.getRestaurantPhotos(
            restaurant.place_id || restaurant.data_id, 
  ...
  private async getActivitiesWithDetails(destination: string, preferences: UserPreferences): Promise<any[]> {
    try {
      ...
      return await Promise.all(
        (data.local_results || []).slice(0, 12).map(async (activity: any) => {
          const photos = await this.getActivityPhotos(
            activity.place_id || activity.data_id, 
  ...
  private async getHotelPhotos(placeId: string, hotelName: string, destination: string): Promise<string[]> {
    try {
  ...
  private async getRestaurantPhotos(placeId: string, restaurantName: string, destination: string): Promise<string[]> {
    try {
      // Try to get photos using place_id first
      if (placeId) {
        const photos = await this.getPlacePhotos(placeId);
        if (photos.length > 0) return photos.slice(0, 2);
      }
  ...
  private async getDestinationPhotos(destination: string): Promise<string[]> {
    try {
      const queries = [
  ...
  private async generateSimpleDailyItinerary(destination: string, duration: number, preferences: UserPreferences): Promise<DayItinerary[]> {
  ...
  private async enhanceDaysWithRealData(days: any[], destination: string): Promise<DayItinerary[]> {
  ...
  private async getActivityPhotos(placeId: string, activityName: string, destination: string): Promise<string[]> {
    try {
      // Try to get photos using place_id first
      if (placeId) {
        const photos = await this.getPlacePhotos(placeId);
  ...
  private async enhanceMealsWithRealData(meals: any[], destination: string): Promise<Meal[]> {
    return Promise.all(meals.map(async (meal: any) => {
      try {
        ...
        const photos = await this.getRestaurantPhotos(
          restaurant?.place_id || restaurant?.data_id,
          restaurant?.title || meal.restaurant,
          destination
        );
  ...
  private calculateDayCost(activities: any[], meals: any[]): number {
  ...
  private getDefaultDestinationInfo(destination: string): DestinationInfo {
    return {
  ...
  private parseDestinationInfo(aiResponse: string): DestinationInfo {
    try {
  ...
  private calculateTotalBudget(days: DayItinerary[], hotels: Hotel[]): number {
  ...
  private getFallbackItinerary(destination: string, duration: number, preferences: UserPreferences): DetailedItinerary {
