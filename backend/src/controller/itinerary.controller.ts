...
import { TripadvisorService } from '../service/tripadvisor.service';
import { MapService } from '../service/map.service';
import { PexelsService } from '../service/pexels.service';
import { PlaceType } from '@prisma/client';

const placesSyncService = new PlacesSyncService();
      ...
      if (!coordinates) {
        ...
        try {
          venues = await placesSyncService.syncPlacesForDestination({
            ...
            interests,
            budgetRange: budgetRange as 'low' | 'mid' | 'high',
        ...
        const images = venues.filter((v: any) => v.imageUrl).map((v: any) => v.imageUrl).slice(0, 10);
        ...
        return res.status(200).json({
          ...
          data: {
            ...
            images,
            summary: {
              totalCost: itinerary.reduce((sum: number, day: any) => sum + day.estimatedCost, 0),
              accommodations: itinerary[0]?.accommodation ? [itinerary[0].accommodation] : [],
              highlights: itinerary.map((day: any) => day.theme),
...
function calculateDetailedDayCost(schedule: any[]): number {
  return schedule.reduce((total, item) => {
    if (item.type === 'meal') {
      const priceRange = item.venue.priceRange || '$$';
      const mealCosts = { '$': 15, '$$': 35, '$$$': 65, '$$$$': 120 };
      return total + (mealCosts[priceRange as keyof typeof mealCosts] || 35);
    }
...
function getDayTheme(day: number, totalDays: number): string {
  if (day === 1) return 'Arrival & City Introduction';
  if (day === totalDays) return 'Final Exploration & Departure';
  if (day === 2) return 'Cultural Immersion';
  if (day === 3) return 'Local Experiences';
  return `Discovery Day ${day}`;
}

function getDayTips(day: number): string[] {
  const tips = [
...
function getTransportationTips(): string[] {
  return [
...
function getBestTimeToVisit(city: string, country: string): string {
  const seasonalInfo: {[key: string]: string} = {
...
function getWeatherTips(): string[] {
  return [
...
function getPackingList(interests: string[], days: number): string[] {
  const baseItems = [
  ...
  const interestItems: {[key: string]: string[]} = {
  ...
  const additionalItems = interests.flatMap(interest => 
...
async function getDestinationCoordinates(destination: string): Promise<{lat: number, lng: number} | null> {
  try {
...
function generateFallbackVenues(city: string, country: string, days: number) {
  const venues = [];
...
async function generateCoverImage(city: string, country: string): Promise<string> {
  try {
    const images = await PexelsService.searchImages(`${city} ${country} landmark`, 1);
    if (images.length > 0) {
      return images[0];
    }
  } catch (error) {
...
async function generateDestinationImages(city: string, country: string, interests: string[]): Promise<string[]> {
  const images: string[] = [];
  ...
  try {
    ...
    for (const query of queries.slice(0, 3)) {
      try {
        const queryImages = await PexelsService.searchImages(query, 2);
